...
Screen('DrawText', wPtr, 'Indicate the direction of the red target', 100, 400);
...
targetCalMatrix = horzcat(targetCalMatrix, zeros(tarCalLength, 2));
[letterPosition, texture, sizes, mask] = loadTextures(screenInfo, wPtr, size(targetCalMatrix,1), black, red);
...
for i = 1:tarCalLength
    ...
    [timeStamp2 a b c d] = Screen('Flip',wPtr, start + SOA + curDur - .5 * frameDur); 
    Screen('FillOval', wPtr, black, Lrect);
    Screen('FillOval', wPtr, black, Rrect);
    ...
    targetCalMatrix(i,4) = timeStamp2 - timeStamp1;
    if (accuracy)
